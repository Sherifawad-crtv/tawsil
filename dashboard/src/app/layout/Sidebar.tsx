...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:sticky top-0 left-0 z-40 md:z-0 h-screen w-64 md:w-56 flex-shrink-0 flex flex-col bg-nav-bg border-r border-border transition-transform duration-300 md:translate-x-0 ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between flex-shrink-0 px-4 h-14">
          <div>
            <h2 className="text-base leading-none text-navy" style={{ fontFamily: "var(--font-heading)" }}>
              TAWSIL
            </h2>
            <p className="mt-1 text-[10px] leading-none text-muted" style={{ fontFamily: "var(--font-mono)" }}>
              Admin Dashboard
            </p>
          </div>
          <button
        ...
        <nav className="flex-1 px-2.5 py-3 flex flex-col gap-0.5 overflow-y-auto">
          {NAV_ITEMS.map(({ to, label, icon: Icon, end }) => (
            <NavLink
              ...
              onClick={onClose}
              className={({ isActive }) =>
                `flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg text-[13px] cursor-pointer transition-all ${
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  >
                    <Icon size={14} strokeWidth={2} color={isActive ? "#1253FA" : "#6B7280"} />
                  </div>
                  {label}
        ...
        <div className="flex-shrink-0 px-4 pb-4">
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 bg-navy">
              <span className="text-white text-[11px]" style={{ fontFamily: "var(--font-heading)" }}>AK</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="truncate text-navy text-xs font-semibold" style={{ fontFamily: "var(--font-sub)" }}>Ahmed Khan</p>
