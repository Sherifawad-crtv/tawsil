...
import { ArrowLeftIcon, UserIcon, MapPointIcon, CameraIcon, UserPlusRoundedIcon, Pen2Icon, ForbiddenIcon, RoutingIcon, RestartIcon } from "@solar-icons/react/bold-duotone";
...
// Width of the card panel floating over the map's left edge.
const PANEL_PX = 380;
...
export default function OrderDetail() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/*
        The map is the page - full width, tall - and the cards float inside
        it as a side panel on the left, scrolling on their own over the map
        the way a maps app does. The vehicle card pins bottom-right, zoom
        top-right, so nothing overlaps anything.
      */}
      <div
        className="relative rounded-2xl overflow-hidden border border-border bg-tile"
        style={{ height: "calc(100vh - 120px)", minHeight: "640px" }}
      >
        <Suspense fallback={null}>
          <OrderMap
            route={route}
            vehicle={tracked?.position ?? null}
            className="h-full w-full"
            clearTopLeft={[PANEL_PX + 48, 48]}
            clearBottomRight={[330, 190]}
          />
        </Suspense>

        {/* Side panel: the cards, over the map. */}
        <div
          className="absolute left-4 top-4 bottom-4 z-[1000] overflow-y-auto pr-1"
          style={{ width: `${PANEL_PX}px` }}
        >
          ...
          {tracked && driver && vehicle && (
            <div className="absolute right-4 bottom-4 z-[1000] w-72 rounded-2xl bg-white border border-border shadow-md p-3.5 flex flex-col gap-2.5">
