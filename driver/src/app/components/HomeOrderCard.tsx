...
/**
 * The carousel's card - same route dot-timeline + meta-chip body as
 * TripCard, but the bottom action differs: a new request (Assigned) needs a
 * deliberate slide to accept, not a tap, since a stray touch while swiping
 * the carousel could otherwise accept an order by accident. Everything
 * except that drag control still navigates to the full order on tap.
 */
export default function HomeOrderCard({ order, onAccepted }: { order: Order; onAccepted: () => void }) {
  ...
  return (
    <div className="rounded-[20px] overflow-hidden bg-white" style={{ boxShadow: "0 8px 28px rgba(4,0,51,0.16)" }}>
      <div
        role="button"
        tabIndex={0}
          ...
          <div className="px-5 pb-4">
            <div className="w-full h-2 rounded-full overflow-hidden" style={{ backgroundColor: "#F0F0EE" }}>
              <div
                ...
                style={{ width: `${stage.pct * 100}%`, background: "linear-gradient(90deg, #040033, #1253FA)", transition: "width 1s ease" }}
              />
            </div>
      ...
      <div className="px-5 pb-4 pt-1" onClick={(e) => e.stopPropagation()}>
        {pending ? (
          <SlideToAccept onAccept={onAccepted} />
        ) : (
          <button
            onClick={() => navigate(`/orders/${order.id}`)}
            ...
            style={{ backgroundColor: "#1253FA", border: "none" }}
          >
            <span style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 700, fontSize: "13px", color: "white" }}>Continue Trip</span>
