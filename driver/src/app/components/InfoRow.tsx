...
export function InfoRow({ icon, label, value }: { icon: ReactNode; label: string; value: ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3 py-2">
      ...
      <span className="text-right" style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 600, fontSize: "13px", color: "#040033" }}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="rounded-[18px] p-3.5 flex items-center gap-3" style={{ backgroundColor: "#F5F5F3" }}>
      ...
      <div className="min-w-0">
        ...
        {subtitle && <p className="truncate" style={{ fontFamily: "'Courier Prime', monospace", fontSize: "11px", color: "#6B7280" }}>{subtitle}</p>}
      </div>
    </div>
  );
}
